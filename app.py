...
class Me:
    ...
    def __init__(self):
        ...
        self.rag_processor = RAGProcessor(self.llm_client)
    ...
    def _initialize_llm_client(self):
        """Initialize LLM client based on available API keys"""
        # Check for different API keys and initialize appropriate client
        if os.getenv("OPENAI_API_KEY"):
            ...
            raise ValueError(
                "No API key found! Please set one of: OPENAI_API_KEY, GEMINI_API_KEY, "
                "ANTHROPIC_API_KEY, or OLLAMA_BASE_URL in your .env file"
            )

    def _get_model_name(self):
        """Get appropriate model name based on the client type"""
        base_url = str(self.llm_client.base_url) if self.llm_client.base_url else ""
...
If the user is engaging in discussion, try to steer them towards getting in touch via email; ask for their email and record it using your record_user_details tool. "
    ...
    def chat(self, message, history):
        ...
        while iteration < max_iterations:
            # Determine model based on the client type
